...
from lobesia_risk_network import RiskClassifier
from conf_db import *
...
def lobesia_risk(update, context):
    # Estrazione delle ultime misurazioni per quel cluster
    sample_dict = extract_last_doc()
    temperature = sample_dict["temperature"]

    try:
        # Converti la temperatura in tensore e normalizza per il modello
        temp_tensor = torch.tensor([[float(temperature)]], dtype=torch.float32)

        # Assicurati che il modello sia in modalità di valutazione
        model.eval()

        # Disabilita la computazione del gradiente durante la valutazione
        with torch.no_grad():
            # Passa i dati al modello per ottenere una previsione
            output = model(temp_tensor)
            _, predicted_class = torch.max(output, 1)

        # Mappa la previsione alla classe di rischio
        risk_mapping = {0: "BASSO", 1: "MEDIO", 2: "ALTO"}
        risk_level = risk_mapping.get(predicted_class.item(), "NON VALIDO")

        # Invia il risultato al bot
        context.bot.send_message(chat_id=update.effective_chat.id, text=f"Rischio lobesia: {risk_level}")
...
def compare_clusters_by_humidity(cluster1, cluster2):
    ...
    if avg_humidity_cluster1 is None and avg_humidity_cluster2 is None:
        ...
        if avg_humidity_cluster1 < avg_humidity_cluster2:
            return f"The cluster {cluster1} has a HEALTHIER status, with lower average humidity ({avg_humidity_cluster1}) compared to the cluster {cluster2} ({avg_humidity_cluster2})."
        else:
            return f"The cluster {cluster2} has a HEALTHIER status, with lower average humidity ({avg_humidity_cluster2}) compared to the cluster {cluster1} ({avg_humidity_cluster1})."
...
def help_command(update, context):
    if is_user_authorized(update, context):
        ...
        /dati - Mostra i dati dell'ultimo documento inserito
        /listclusters - Lista i cluster disponibili nel database
        /calculate_average - Calcola le medie di umidità, temperatura, ecc. per un cluster
        /compare_clusters - Compara l'umidità media di due cluster (specifica i nomi dei cluster come argomenti)
        /lobesia_risk - Calcola il rischio di lobesia in base alla temperatura
        /cancel - Cancella l'operazione in corso
        """
        context.bot.send_message(chat_id=update.effective_chat.id, text=commands)

# Aggiunta dell'handler per il comando /help
